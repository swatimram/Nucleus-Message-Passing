...
main(int argc, char **argv)
{
    ...
    ListIterator<char*> *li = new ListIterator<char*> (userProgName);
    List <Thread*> *threads = new List<Thread*>();
    //<Thread*>;
    while (!li->IsDone()) {
        threads->Append(new Thread(li->Item()));
	li->Next();    
    }
    ListIterator<Thread*> *ti = new ListIterator<Thread*> (threads);
    while (!ti->IsDone()) {
	
        std::string tname = "";
	tname =  ti->Item()->getName() ;
        char * progName = new char[tname.size() + 1];
        std::copy(tname.begin(), tname.end(), progName);
        progName[tname.size()] = '\0';
       
        ti->Item()->Fork((VoidFunctionPtr)RunUserProg, (void *)progName);
        ti->Next();    
    }
